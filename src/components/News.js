...
const News = (props) => {
 ...
 const [totalResults, setTotalResults] = useState(0);
  ...
  useEffect(() => {
    updateNews();
    document.title = `NewsMoney - ${capitalizeFirstLetter(props.category)}`;
  ...
  , [])
    ...
    return (
      <>
          <h1 className="text-center" style={{margin : '35px 0px' ,marginTop : '90px'}}>Top {capitalizeFirstLetter(props.category)} Headlines - NewsMonkey!</h1>
         {loading && <Spinner />}
        ...
        >
          ...
          <button disabled={this.state.page+1>Math.ceil(this.state.totalResults/props.pageSize)} type="button" className="btn btn-dark" onClick={this.nextClickHandler}>Next &raquo;</button>
          </div> */}
